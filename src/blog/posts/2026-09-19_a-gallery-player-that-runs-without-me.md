...
The vj-show repo is private for now while it gets its first real installation. The roadmap in it has the program/preview switcher with a T-bar next, and the reason a manual T-bar across tiles is harder than it looks: a slider value that arrives on tile B a few milliseconds after tile A shows at the seam mid-fade, so the bar has to be sent as timed samples and interpolated, the same way game netcode moves other players.
